...
class ShopTopProduct extends StatefulWidget {
  final int id;
  const ShopTopProduct({Key? key, required this.id}) : super(key: key);

  @override
  State<ShopTopProduct> createState() => _ShopTopProductState();
}

class _ShopTopProductState extends State<ShopTopProduct> {
  @override
  void initState() {
    getShopTop(context, widget.id);
    super.initState();
  }
  ...
  getShopTop(context, int id) {
    BlocProvider.of<ShopProductBloc>(context).add(GetShopTopProduct(id));
  }
}

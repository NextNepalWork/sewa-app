...
class ShopBrandProduct extends StatefulWidget {
  final int id;
  const ShopBrandProduct({Key? key, required this.id}) : super(key: key);

  @override
  State<ShopBrandProduct> createState() => _ShopBrandProductState();
}

class _ShopBrandProductState extends State<ShopBrandProduct> {
  @override
  void initState() {
    getShopBrands(context, widget.id);
    super.initState();
  }
  ...
  getShopBrands(context, int id) {
    BlocProvider.of<ShopProductBloc>(context).add(GetShopBrandProduct(id));
  }
}
